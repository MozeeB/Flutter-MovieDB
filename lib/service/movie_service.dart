...
import 'package:flutter_movie_db/model/actor.dart';
...
abstract class MovieDB {
  ...
  Future<List<Movie>> topRatedMovies({int page = 1});
  Future<List<Movie>> popularMovies({int page = 1});
  Future<List<Movie>> nowPlayingMovies({int page = 1});
...
class _MovieRepository extends MovieDB{
  ...
  @override
  Future<List<Movie>> nowPlayingMovies({int page = 1}) async {
    var url = Uri.https(MOVIE_DB_BASE_URL, '/3/movie/now_playing',
        { 'api_key': API_KEY,
          'language': 'en-US',
          'page': page.toString(),
  ...
  @override
  Future<List<Movie>> topRatedMovies({int page = 1}) async{
    var url = Uri.https(MOVIE_DB_BASE_URL, '/3/movie/top_rated',
        { 'api_key': API_KEY,
          'language': 'en-US',
          'page': page.toString(),
  ...
  Future<List<Movie>> popularMovies({int page = 1}) async {
    var url = Uri.https(MOVIE_DB_BASE_URL, '/3/movie/popular',
        { 'api_key': API_KEY,
          'language': 'en-US',
          'page': page.toString(),
